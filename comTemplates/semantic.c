	...
	switch (v->kind) {
		...
		case A_simpleVar: {
			if (!(S_look(venv, v->u.simple))) {
				printf("\n\n Varialvel utilizada é inexistente.\n");
				exit(1);
			}
	...
	switch (a->kind) {
		...
		case A_varExp:
    		...
    		case A_callExp: {
    			...
    			if (!(funnn)) {
				printf("\n\n Função utilizada é inexistente.\n");
				exit(1);
			}
			...
			A_fieldList params = funnn->params;
    			...
    			while ((args != NULL) && (params != NULL)) {
    				...
    				if (strcmp(Ty_to_String(tipoArg), S_name(fld->typ))) {
    					printf("\n\n O tipo de um parâmetro de uma das funções não corresponde ao do código.\n");
    					exit(1);
    				}
			...
			direita = transExp(a->u.op.right);
    			...
    			case A_ifExp: {
    				...
    				if (then_ty.ty != else_ty.ty) {
    					...
    					if (a->u.iff.elsee) {
    						...
    						printf("\n\n Quando não tem else, o then não pode produzir valor.");
    					}
    			...
    			case A_whileExp: {
    				transExp(a->u.whilee.test);
    				transExp(a->u.whilee.body);
			...
			A_fundec func = d->u.function->head;
				...
				A_field fld = func->params->head;
    				...
    				if (strcmp(Ty_to_String(tipoExp), S_name(aux->head->result))) {
    					printf("\n\n O corpo da função retorna um valor de tipo diferente do que o declarado.\n");
    					exit(1);
    				}
